...
def _json_interface_to_text(name: str, data: dict) -> str:
    """Render one AOS-CX interface's REST/JSON config (merged Interface+Port dict)
    as AOS-CX-style pseudo-CLI text, so the substring-based ``config_contains`` /
    ``vlan_check`` checks evaluate the same way they do for CLI configs.

    Field names are the REAL AOS-CX ``running-config`` keys (verified live), NOT a
    flat CLI dump: description lives in the ``Interface`` section; vlan_mode/
    vlan_tag/vlan_trunks/stp_config/loop_protect_enable/admin in the ``Port``
    section (booleans arrive as the strings "true"/"false").
    """
    ...
    # Admin state (AOS-CX uses "up"/"down"; default to enabled when unspecified).
    if str(data.get("admin", "")).strip().lower() == "down":
        lines.append("    shutdown")
    else:
        lines.append("    no shutdown")

    # VLAN. access → "vlan access N"; trunk / native-tagged / native-untagged →
    # "vlan trunk ..." (the vlan_check trigger keys purely on "access"/"trunk").
    ...
    # Spanning tree (AOS-CX stp_config) + loop protection.
...
def _find_aos_entry(section, interface_name: str) -> dict:
    """Look up one interface in an AOS-CX config section (keys are URL-encoded,
    e.g. ``1%2F1%2F14``). Tries the encoded key, the plain name, a match on the
    entry's ``name`` field, then an unquote of each key."""
    if not isinstance(section, dict):
        return {}
    enc = quote(interface_name, safe="")
    if enc in section and isinstance(section[enc], dict):
        return section[enc]
    if interface_name in section and isinstance(section[interface_name], dict):
...
def aos_cx_json_interface(data: dict, interface_name: str) -> str:
    ...
    Interface config is split across the ``Interface`` (description) and ``Port``
    (vlan/stp/admin) top-level sections; merge both. Falls back to treating the
    whole object as the interface map (handles a flatter top-level layout too)."""
    ports = data.get("Port") if isinstance(data.get("Port"), dict) else data
    ...
    if not port and not iface:
        return ""
    merged = {**iface, **port}  # Port (vlan/stp) wins; description comes from Interface
    return _json_interface_to_text(interface_name, merged)
...
def get_interface_config(device, interface_name: str) -> str:
    ...
    JSON running-config (stored by collect_aos_cx_config), which would otherwise
    never yield an ``interface <name>`` stanza and always return ''."""
