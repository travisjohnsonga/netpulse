...
# Daily fleet compliance run: check frequently (the task is hour-gated at
...
class Command(BaseCommand):
    ...
    def _collect_unifi_telemetry(self):
    ...
    def _run_due_compliance(self):
        # Daily fleet compliance run (hour-gated at COMPLIANCE_RUN_HOUR, default
        # 03:00 — after the config backup — and same-day deduped). Stores each
        # device's weighted DeviceComplianceScore. Per-device config collection
        # already re-runs compliance on its own; this catches drift independent
        # of config changes (interface/role rules, OS lifecycle).
