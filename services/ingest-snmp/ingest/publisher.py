"""
NATS JetStream publisher for SNMP metrics and trap events.

Subjects:
  netpulse.telemetry.<device_id>.metrics  — poll results
  netpulse.telemetry.<device_id>.trap     — received traps / informs
"""
import json
import logging
import re
...
class NATSPublisher:
    ...
    async def publish_metrics(self, device_id: str, payload: dict[str, Any]) -> None:
        subject = f"{self._prefix}.{_token(device_id)}.metrics"
        await self._pub(subject, payload)

    async def publish_trap(self, device_id: str, payload: dict[str, Any]) -> None:
        subject = f"{self._prefix}.{_token(device_id)}.trap"
        await self._pub(subject, payload)
