    ...
    <div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-5 space-y-4 max-w-xl">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">Shown in the UI header, emails and reports.</p>
        </div>
        <div>
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Default telemetry retention (days)</label>
          <input className={inputCls} type="number" value={retention} onChange={(e) => setRetention(e.target.value)} />
          <p className="text-xs text-gray-400 mt-1">Per-data-type overrides live under System → Data retention.</p>
        </div>
        <button
          onClick={save}
