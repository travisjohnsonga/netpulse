...
export default function ConfigBackupSection() {
  ...
  return (
    <section className="space-y-3">
      <div>
        <h2 className="text-lg font-semibold text-gray-800">Configuration Backup</h2>
        <p className="text-sm text-gray-500">Where device configs are stored and optionally mirrored to git.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <p className="text-xs text-gray-400">Used: {gb(s.local_used_bytes)}</p>
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-4 space-y-3">
          ...
          <div className={clsx('space-y-3', !s.git_enabled && 'opacity-50 pointer-events-none')}>
            ...
            <div className="flex gap-2">
              <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">Repository URL</label><input className={inputCls} value={s.git_repo_url} onChange={(e) => set({ git_repo_url: e.target.value })} placeholder="https://github.com/org/configs" /></div>
              <div className="w-28"><label className="block text-xs text-gray-500 mb-1">Branch</label><input className={inputCls} value={s.git_branch} onChange={(e) => set({ git_branch: e.target.value })} /></div>
            </div>
            <div>
            ...
            <div>
              <label className="block text-xs text-gray-500 mb-1">Token / Key</label>
              ...
              <p className="text-xs text-gray-400 mt-1">🔒 Git credentials stored in OpenBao.</p>
            </div>
            <div className="flex gap-2">
              <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">Commit author</label><input className={inputCls} value={s.git_commit_author} onChange={(e) => set({ git_commit_author: e.target.value })} /></div>
              <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">Commit email</label><input className={inputCls} value={s.git_commit_email} onChange={(e) => set({ git_commit_email: e.target.value })} placeholder="netpulse@company.com" /></div>
            </div>
            <div>
          ...
          {testResult && (
            <div className={clsx('rounded-lg px-3 py-2 text-xs border', testResult.ok ? 'bg-green-50 border-green-200 text-green-800' : 'bg-red-50 border-red-200 text-red-800')}>
              {testResult.ok ? '✅' : '❌'} {testResult.message}
            </div>
          )}

          <div className="flex gap-2">
            <button onClick={doTest} disabled={busy === 'test'} className="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50">{busy === 'test' ? 'Testing…' : 'Test Connection'}</button>
            <button onClick={doSync} disabled={busy === 'sync'} className="px-3 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50 disabled:opacity-50">{busy === 'sync' ? 'Syncing…' : 'Sync Now'}</button>
