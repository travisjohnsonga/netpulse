...
function urgency(days: number): { cls: string; text: string } {
  if (days < 0) return { cls: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', text: `${Math.abs(days)}d ago` }
  if (days < 180) return { cls: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400', text: `${days}d left` }
  if (days < 365) return { cls: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400', text: `${days}d left` }
  return { cls: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', text: `${days}d left` }
}
...
export default function Lifecycle({ device }: { device: DeviceDetail }) {
  ...
  if (milestones.length === 0) {
    return <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700"><EmptyState title="No lifecycle data" description="No EOL/EOS milestones are recorded for this device's model yet." icon="📅" /></div>
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          <p className="text-xs text-gray-400 dark:text-gray-500">Days until end of security support</p>
          {eosl ? (
            <>
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{TYPE_LABEL[eosl.milestone_type]} · {eosl.milestone_date}</p>
            </>
          ) : <p className="text-gray-400 dark:text-gray-500 mt-2">—</p>}
        </div>
      </div>

      {/* All milestones */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-700"><h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100">Milestones</h3></div>
            ...
            return (
              <div key={m.id} className="flex items-center gap-4 px-5 py-3">
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-gray-800 dark:text-gray-100">{TYPE_LABEL[m.milestone_type]}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">{m.milestone_date}{m.source && ` · ${m.source}`}</p>
                </div>
      ...
      {/* Recommended actions */}
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <h3 className="text-sm font-semibold text-gray-800 dark:text-gray-100 mb-2">Recommended Actions</h3>
        <ul className="space-y-1.5 text-sm text-gray-700 dark:text-gray-300 list-disc list-inside">
...
function Group({ title, milestones }: { title: string; milestones: LifecycleMilestone[] }) {
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
      <p className="text-xs text-gray-400 dark:text-gray-500 mb-2">{title} lifecycle</p>
      {milestones.length === 0 ? <p className="text-sm text-gray-400 dark:text-gray-500">No data</p> : (
        <ul className="space-y-1">
          {milestones.map((m) => {
            const u = urgency(daysUntil(m.milestone_date))
            return (
              <li key={m.id} className="flex items-center justify-between text-sm">
                <span className="text-gray-600 dark:text-gray-400">{TYPE_LABEL[m.milestone_type]}</span>
