      ...
      {!showForm ? (
        <div className="flex items-center justify-between gap-3 bg-green-50 border border-green-200 rounded-lg px-3 py-2.5">
          <div className="min-w-0">
            <p className="text-sm font-medium text-green-800 truncate">✅ {label}</p>
            <p className="text-xs text-green-700">🔒 Stored securely in OpenBao</p>
          </div>
          <button onClick={() => { reset(); setEditing(true) }}
            className="shrink-0 px-2.5 py-1 text-xs border border-green-300 bg-white rounded-md hover:bg-green-100 text-green-800 font-medium">
            Update Key
          </button>
        </div>
      ) : (
        <div className="space-y-3">
          ...
          <div className={clsx('grid gap-3', fields.length > 1 ? 'grid-cols-2' : 'grid-cols-1')}>
            {fields.map((f) => (
              <div key={f.key}>
                {fields.length > 1 && <label className="block text-xs text-gray-500 mb-1">{f.label}</label>}
                <input
          ...
          <p className="text-xs text-gray-400">🔒 Stored securely in OpenBao — never displayed after saving.</p>
        </div>
      )}
    </div>
  )
