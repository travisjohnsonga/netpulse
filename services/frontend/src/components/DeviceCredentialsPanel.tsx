    ...
    <div className="fixed inset-0 z-50 flex justify-end bg-black/40" onClick={onClose}>
      <div className="bg-white dark:bg-gray-800 w-full max-w-md h-full shadow-xl flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100">Device Credentials</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {msg && <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-700 rounded-lg px-3 py-2 text-sm text-green-700 dark:text-green-400">{msg}</div>}

          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Credential Profile</label>
            <select
              className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={selectedId ?? ''} onChange={(e) => { setSelectedId(e.target.value ? Number(e.target.value) : null); setTest(null) }}>
              <option value="">— No profile —</option>
              {profiles.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            {profiles.length === 0 && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">No profiles — <Link to="/settings/credentials" className="text-blue-600 hover:text-blue-800">create one</Link>.</p>
            )}
          </div>

          {detail && (
            <div>
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Profile capabilities</p>
              <ul className="space-y-1 text-sm">
                {caps(detail).map((c) => (
                  <li key={c.label} className={clsx('flex items-center gap-2', c.on ? 'text-gray-800 dark:text-gray-100' : 'text-gray-400 dark:text-gray-500')}>
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-gray-200 dark:border-gray-700">
          <button onClick={runTest} disabled={selectedId == null || test === 'running'} className="px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 disabled:opacity-50 dark:text-gray-300">
