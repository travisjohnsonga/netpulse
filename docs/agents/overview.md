...
enrolls into NetPulse with a one-time token, receives a unique client
certificate from NetPulse's internal CA, and pushes CPU / memory / disk /
...
- Auto-enrolls the server into the NetPulse inventory (and the Servers page)
- Single static binary (the Linux core build is stdlib-only)

## Architecture

```
 Agent  ──  mTLS, outbound 443  ──▶  NetPulse (nginx → api)
```
...
- Authenticates with a client certificate issued by NetPulse's CA (OpenBao PKI)
