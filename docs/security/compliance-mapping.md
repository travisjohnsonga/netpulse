...
| **A.8.24** Use of cryptography / key management | **Met** | All secrets in OpenBao (hvac KV v2); TLS 1.2/1.3 external with Mozilla-intermediate ciphers (1.2 floor); per-agent EC P-384 PKI certificates; JWT HS256. Note: key rotation is OpenBao-supported but operationally manual (no automated rotation policy). | `apps/credentials/vault.py`; `services/frontend/nginx.conf`; `apps/agents/pki.py`; [Secrets](secrets.md). |
